...
python -m app.cli create-tables | seed-slices | seed-demo | release-due
"""
    ...
    async with get_sessionmaker()() as session:
        for spec in DEMO_SELLERS:
            ...
            if user.body_profile is None:
                user.body_profile = BodyProfile(
                    user_id=user.id, source_per_field={}, picker_choices={}
                )
            ...
            existing = {
                row.title
                for row in (
                    await session.scalars(select(Listing).where(Listing.seller_id == user.id))
                ).all()
            }
            for title, cat, brand, size, price, meas in spec["listings"]:
                if title in existing:
                    continue
                photo_url = f"https://picsum.photos/seed/{abs(hash(title)) % 1000}/900/1200"
                listing = Listing(
                    seller_id=user.id,
                    title=title,
                    ...
                    size_label=size,
                    price_pence=price,
                    photos=[{"url": photo_url, "is_modelled_fit": True}],
                    status=ListingStatus.live,
                )
                listing.measurements = GarmentMeasurements(**meas)
                session.add(listing)
        await session.commit()
    n_listings = sum(len(s["listings"]) for s in DEMO_SELLERS)
    print(f"seeded {len(DEMO_SELLERS)} sellers, {n_listings} listings")
